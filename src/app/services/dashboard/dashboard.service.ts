...
export class DashboardService {
  ...
  incomesDataProcess(incomeMovementsPerMonth: MonthMovements[]) {
    incomeMovementsPerMonth.forEach(monthMovements => {
      this.getDataForIncomesMain(
        monthMovements.month,
  ...
  incomesFillingDataForNormalMovs(
    concept: Concept,
    movement: Movement,
    month: number
  ) {
    let arrayOfMonth = this.incomesData.filter(
    ...
    if (arrayOfMonth.length == 0) {
        ...
        data: [
          {
            ...
            backgroundColor: this.getCategoryById(concept.category.parent.id)
              .color,
            details: [
              {
      ...
      this.incomesData.forEach(element => {
        element.month == month
          ? this.incomeElementWithMonthCreated(element, concept, movement)
          : null;
      });
    }
  }

  incomeElementWithMonthCreated(
    element: ResumeMainData,
    concept: Concept,
    movement: Movement
  ) {
  ...
  incomesFillingDataForMovsWoSubcats(
    concept: Concept,
    movement: Movement,
    month: number
  ) {
    ...
    if (arrayOfMonth.length == 0) {
      ...
      this.incomesData.forEach(element => {
        element.month == month
          ? this.incomeElementWithMonthCreatedWoSubcat(
              element,
              concept,
              movement
            )
  ...
  incomeElementWithMonthCreatedWoSubcat(
    element: ResumeMainData,
    concept: Concept,
    movement: Movement
  ) {
  ...
  incomesFillingDataForMovsWoCats(movement: Movement, month: number) {
    ...
    if (arrayOfMonth.length == 0) {
      ...
      this.incomesData.forEach(element => {
        element.month == month
          ? this.incomeElementWithMonthCreatedWoCat(element, movement)
          : null;
      });
    }
  }

  incomeElementWithMonthCreatedWoCat(
    element: ResumeMainData,
    movement: Movement
  ) {
    let categoryAlreadyExists: boolean = false;
          ...
          if (!isNullOrUndefined(concept.category)) {
            if (!isNullOrUndefined(concept.category.name)) {
              if (!isNullOrUndefined(concept.category.parent)) {
                ...
                this.expensesFillingDataForMovsWoSubcats(
                  concept,
                  movement,
                  month
                );
  ...
  expensesFillingDataForNormalMovs(
    concept: Concept,
    movement: Movement,
    month: number
  ) {
    let arrayOfMonth = this.expensesData.filter(
      element => element.month == month
    );
    let movements: Movement[] = [];
    movements.push(movement);
    if (arrayOfMonth.length == 0) {
      ...
      this.expensesData.forEach(element => {
        element.month == month
          ? this.expensesElementWithMonthCreated(element, concept, movement)
          : null;
      });
    }
  }

  expensesElementWithMonthCreated(
    element: ResumeMainData,
    concept: Concept,
    movement: Movement
  ) {
  ...
  expensesFillingDataForMovsWoSubcats(
    concept: Concept,
    movement: Movement,
    month: number
  ) {
    ...
    if (arrayOfMonth.length == 0) {
      ...
      this.expensesData.forEach(element => {
        element.month == month
          ? this.expensesElementWithMonthCreatedWoSubcat(
              element,
              concept,
              movement
            )
  ...
  expensesElementWithMonthCreatedWoSubcat(
    element: ResumeMainData,
    concept: Concept,
    movement: Movement
  ) {
  ...
  expensesFillingDataForMovsWoCat(movement: Movement, month: number) {
    ...
    if (arrayOfMonth.length == 0) {
      ...
      this.expensesData.forEach(element => {
        element.month == month
          ? this.expensesElementWithMonthCreatedWoCat(element, movement)
          : null;
      });
    }
  }

  expensesElementWithMonthCreatedWoCat(
    element: ResumeMainData,
    movement: Movement
  ) {
    let categoryAlreadyExists: boolean = false;
  ...
  dataForBalanceChart() {
    ...
    this.movementsList.forEach(movement => {
      ...
      if (auxMonth == dateMovementMonth) {
        ...
        this.dataProcessBalanceChart(
          auxMonth,
          this.chargeBalanceAux,
          this.depositBalanceAux
        );
  ...
  dataProcessBalanceChart(
    ...
    this.savingBalanceAux < 0
      ? (this.savingBalanceAux = 0)
      : (this.savingBalanceAux = this.savingBalanceAux);
