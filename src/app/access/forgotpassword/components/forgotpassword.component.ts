...
export class ForgotpasswordComponent implements OnInit {
  ...
  onSubmit() {
    this.passwordService
      .createForgotPasswordToken(this.forgotPasswordForm.value.email)
      .subscribe(
        res => {
          res.status === 200
            ? (this.success = true)
  ...
  setComponentText() {
    if (this.defaultMessages) {
      ...
      this.header =
        'El link con el que intentaste restablecer tu contraseña ya ha expirado';
      this.body =
        'Vuelve a ingresar el correo electrónico con el que te registraste';
    }
  }
}
